...
With reference to Figure 1, the following steps occur on the execution of the `addappt` command:

<1> The command is first passed to an instance of the `LogicManager` class which in turn executes `AddressBookParser::parseCommand`.

<2> The `AddressBookParser` parses the command word (`addappt`) and executes `AddApptCommandParser::parse`.
...
<5> The `AddApptCommand::execute` method constructs a new `Person` object using all the details of the old `Person` with the exception that the `AppointmentsList` is a copy of the original `Person` 's `AppointmentsList` with an added `Appointment`.
...
<7> Finally, the `AddApptCommand::execute` method terminates, returning a `CommandResult` with a success message. The `LogicManager` then returns the same `CommandResult` as the return value of the `LogicManager::execute` method.

<8> The command execution then ends.
