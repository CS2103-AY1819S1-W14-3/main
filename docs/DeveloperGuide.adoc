...
=== Adding appointment for a patient

. Adds an appointment for a checked in patient

.. Prerequisites: Patient must not have another appointment at the same date and time
...
.. Test case: `addappt ic/S1234567A type/SRG pn/Heart Bypass dt/27-04-2019 10:30 doc/Pepper`
   Expected: Appointment is not added because of missing salutation for Doctor.
