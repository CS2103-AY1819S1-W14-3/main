...
* `DietType`
** This is a `Enum` class representing the three different types of dietary requriements.
...
* Next, the `AddDietCommandParser#parse` method is parse the differnt dietary requirements into one `DietCollection` object. An instance of `AddDietCommand` is returned after the parsing.
...
image::AdddietSequenceDiagram.png[width="650"]
